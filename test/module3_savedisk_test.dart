...
void main() {
  ...
  group('Module 3 - LogController (Save Data to Disk)', () {
    ...
    test('TC01: log data should persist after save and reload', () async {
      ...
      expect(ctrl2.logs[0].title, 'Judul Test',
        reason: 'Title tidak sesuai setelah reload');
    });

    // TC02 ──────────────────────────────────────────────────
    test('TC02: deleted log should not persist after save and reload', () async {
    ...
    // TC03 ──────────────────────────────────────────────────
    test('TC03: loadFromDisk should keep logs empty when storage is empty', () async {
    ...
    // TC04 ──────────────────────────────────────────────────
    test('TC04: encodeToJson should return valid JSON string with title key', () async {
      // (1) Arrange
      final ctrl = LogController();
      await Future.delayed(Duration.zero);
      ...
      expect(result, isA<String>(),
        reason: 'Hasil harus berupa String');
    ...
    test('TC05: decodeFromJson should return correct LogModel list', () async {
      ...
      final ctrl = LogController();
      await Future.delayed(Duration.zero);
      const jsonString = '[{"title":"Judul Test","description":"Deskripsi","timestamp":"2025-01-01T00:00:00.000","category":"Software"}]';
      ...
      expect(result[0].title, 'Judul Test',
        reason: 'Title tidak sesuai');
    });

    // TC06 ──────────────────────────────────────────────────
    test('TC06: encode-decode round-trip should produce identical data', () async {
      // (1) Arrange
      final ctrl = LogController();
      await Future.delayed(Duration.zero);
      final original = [LogModel(
        title: 'Judul Test', description: 'Deskripsi',
        timestamp: DateTime(2025, 1, 1), category: 'Software',
      )];
    ...
    test('TC08: updateLog should change log data at given index', () async {
      ...
      expect(actual, expected,
        reason: 'Expected $expected but got $actual');
      expect(ctrl.logs[0].category, 'Hardware',
